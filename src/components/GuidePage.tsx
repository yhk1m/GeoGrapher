      ...
      <div style={styles.content}>
        ...
        <section style={styles.section}>
          ...
          <h3 style={styles.mapGroupTitle}>
            통계지도 (NEW)
            <span style={styles.newBadge}>세계지도 지원</span>
          </h3>
          <p style={styles.mapGroupDesc}>
            한국 행정구역(시·도, 시·군·구, 시·군·구 광역 통합)과 <strong>세계 국가</strong>(대서양 중심 / 태평양 중심) 기반 SVG 지도 시각화입니다.
            도형표현도·단계구분도·등치선도·유선도를 하나의 도구에서 제작하고 고해상도 PNG로 내보낼 수 있습니다.
            세계지도는 Miller 투영을 사용하며, 태평양 중심은 중앙 경도 150°E(절단선 30°W)로 아프리카·유럽이 온전히 보존됩니다.
            데이터 입력 시 지역명 오름/내림차순 정렬 토글이 제공되어 177개국에서도 원하는 국가를 빠르게 찾을 수 있습니다.
          </p>

          <div style={styles.card} onClick={() => onNavigate('choropleth')}>
            <h3 style={styles.cardTitle}>단계구분도</h3>
            <p style={styles.cardDesc}>
              한국 행정구역(시·도 / 시·군·구 / 광역 통합) 또는 <strong>세계 국가</strong>(대서양·태평양 중심)를 등급별 색상으로 채워 분포를 표현합니다.
          ...
          <div style={styles.card} onClick={() => onNavigate('symbolmap')}>
            ...
            <p style={styles.cardDesc}>
              각 지역(한국 행정구역 또는 세계 국가)의 중심점에 <strong>파이/막대 차트</strong>를 배치하여 복수 항목의 값을 동시에 표현합니다.
          ...
          <div style={styles.card} onClick={() => onNavigate('isoline')}>
            ...
            <p style={styles.cardDesc}>
              지점별 관측값(한국 행정구역 또는 세계 국가 단위)에서 <strong>IDW 보간</strong>으로 등치선을 생성합니다.
              등치선 간격·주곡선/간곡선, 색상 채우기(그라데이션) on/off를 설정할 수 있으며,
              지도 경계로 clip 처리되어 깔끔하게 출력됩니다. 주곡선 숫자 라벨은 등치선 위에 직접 배치됩니다.
            </p>
          </div>
          ...
          <div style={styles.card} onClick={() => onNavigate('flowmap')}>
            ...
            <p style={styles.cardDesc}>
              지역·국가 간 <strong>이동·물류·무역</strong>을 출발→도착 화살표로 표현합니다.
              선 종류(직선/베지어 곡선)와 곡률, 화살표 스타일(삼각/열림/없음), 값에 비례한 선 굵기(최소/최대)를 설정합니다.
              지도를 클릭해 출발·도착을 지정하거나 CSV로 일괄 입력할 수 있으며, 세계지도에서는 국가 간 무역 흐름을 표현하기 좋습니다.
            </p>
          </div>
        </section>
